...
Cuando una línea aparece desconectada (punto rojo), el chip es clickeable:
al tocarlo se abre el código QR como imagen, listo para escanear desde
**WhatsApp → Dispositivos vinculados**, sin tener que entrar a los logs de
Render. Se actualiza solo cada pocos segundos mientras el modal está abierto
—los QR de WhatsApp rotan si no se escanean a tiempo— y se cierra solo en
cuanto la línea queda conectada.

Esto resuelve el caso de que quien construye o administra el sistema no
tenga acceso físico a los teléfonos de Mauricio y Daniel: basta con
compartirles la URL del panel y la contraseña, y cada uno escanea el suyo
...
3. Agrega un disco nuevo y las mismas variables de entorno que ya
   configuraste, más `PANEL_PASSWORD`. El puerto (`PORT`) lo asigna Render
   solo — no hace falta agregarlo a mano en un Web Service.
4. Vas a tener que **volver a vincular las dos líneas**, porque es
   técnicamente un servicio nuevo con un disco nuevo y vacío. Ya no hace
   falta entrar a los logs de Render para esto: abre el panel, y los chips
   de línea desconectada dejan escanear el QR directamente ahí (ver
   "Vincular WhatsApp desde el panel" más arriba).
